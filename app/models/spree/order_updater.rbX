  ...
  class OrderUpdater
    ...
    def update_shipment_state
      if order.backordered?
        ...
        # 12/12/13 DH: Only stock is required for the Lining + Tie-backs so place order with supplier when backordered.
        #              We only need to do this once per order (since the 'shipment_state' is updated several times 
        #              per order process, previously done via 'unless order.shipment_state == 'backorder')
        if order.payment_state == 'paid'
          # 1 - Send request spec to curtain maker
          begin
            OrderMailer.bsc_order_email(order).deliver
          rescue Exception => e
            logger.error("#{e.class.name}: #{e.message}")
            logger.error(e.backtrace * "\n")
          end
          
          # 2 - Add stock, for variant quantities requested in order, back to zero (maybe???)
